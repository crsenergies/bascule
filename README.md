...
One local OpenAI-compatible endpoint in front of all your AI providers. When one fails or hits its rate limit, the request moves to the next one, down to a local Ollama model if you want. Single file, zero dependencies, ~40 MB of RAM.
...
`model` is a combo (`auto`, `fast`, `smart`, `local`), `provider/model` (`groq/llama-3.3-70b-versatile`), or a bare model name.

## Features

- `POST /v1/chat/completions` (streaming and non-streaming), `GET /v1/models`, `GET /stats`, `GET /health`.
- Fallback on 429, 5xx, timeouts and network errors, including in streaming as long as no byte has reached the client.
...
`config.json` lists `providers` (`type`: `openai` or `anthropic`, `baseUrl`, `keys`, `models`) and `combos`. `${VAR}` is replaced by the environment variable. Any OpenAI-compatible service works: add it with its base URL.

## Security

- Listens on `127.0.0.1` only by default.
- Binding to another address (`HOST=0.0.0.0`) is refused unless `BASCULE_KEY` holds 16+ characters.
...
```bash
node test.mjs   # end-to-end against mock providers, no network, no keys
```
